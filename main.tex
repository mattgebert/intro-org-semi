...
%\usepackage[toc,section=chapter]{glossaries} %Allows the use of a glossary. https://mirror.aarnet.edu.au/pub/CTAN/macros/latex/contrib/glossaries/glossariesbegin.pdf
\usepackage[toc, acronym, section=chapter]{glossaries-extra} % Used extras so we can define only first time hyperlinking for an acyronym per chapter. See usage below.
...
%https://tex.stackexchange.com/questions/404051/glossaries-hyperlink-only-at-the-first-occurrence-in-every-chapter
...
%\setacronymstyle{long-short} % Glossaries command - Allow acroynms to be defined, then only used once through the document.
\setabbreviationstyle[acronym]{long-short} % Glossaries-extra command - Allow acroynms to be defined, then only used once through the document.
...
\begin{document}
    ...
    \subfile{chapters/02-polymers.tex}
    \subfile{chapters/04-nexafs.tex}
    \subfile{chapters/05-saxs-waxs.tex}
    \subfile{chapters/06-ReXS.tex}
    \subfile{chapters/07-R-XEFI.tex}
